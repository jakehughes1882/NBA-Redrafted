...
// domains
xScale.domain(d3.extent(data, function(d) { return d.FPG; })).nice();
...
// create svg
var svg = d3.select("#container_kpi_to_bar_fpg")
			.append("svg")
...
var bars = 	svg.selectAll(".bar")
				...
				.append("rect")
					...
					.attr("x", -15) //function(d) { return xScale(Math.min(0, d.FPG)); })
					.attr("width", 0)	
...
var labels = svg.selectAll("text")
			   ...
			   .append("text")
			   .text(function(d) {
			   		return StringMap[d.DerbyFlag] + ": "+(Math.round(d.FPG * 100))/100;
			   })
			   .attr("text-anchor", "start")
			   .attr("y", function(d) { return yScale(d.DerbyFlag) + 5 + (yScale.bandwidth() - 1)/2; })
			   .attr("x", 20) //function(d) { return xScale(Math.min(0, d.FPG)) + 320; }   )
...
window.addEventListener("scroll", function(e) {
	...
	if (last.id === "P10") {		
		...
		svg.selectAll(".bar")
			.transition()
				.attr("width", function(d) { return xScale(d.FPG); })
		labels.transition()
				.attr("opacity", 1)		
		kpi.transition()
				 .attr("opacity", 0)
