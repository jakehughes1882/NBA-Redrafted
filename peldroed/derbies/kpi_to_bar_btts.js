...
// domains
xScale.domain(d3.extent(data, function(d) { return d.BTTS; })).nice();
...
var bars = 	svg.selectAll(".bar")
				...
				.append("rect")
					...
					.attr("x", -15) //function(d) { return xScale(Math.min(0, d.BTTS%)); })
					.attr("width", 0)	
...
var labels = svg.selectAll("text")
			   ...
			   .append("text")
			   .text(function(d) {
			   		return StringMap[d.DerbyFlag] + ": "+(Math.round(d.BTTS * 10000))/100 + "%";
			   })
			   .attr("text-anchor", "start")
			   .attr("y", function(d) { return yScale(d.DerbyFlag) + 5 + (yScale.bandwidth() - 1)/2; })
			   .attr("x", 20) //function(d) { return xScale(Math.min(0, d.BTTS%)) + 320; }   )
...
window.addEventListener("scroll", function(e) {
	...
	if (last.id === "P10") {		
		...
		svg.selectAll(".bar")
			.transition()
				.attr("width", function(d) { return xScale(d.BTTS); })
		labels.transition()
				.attr("opacity", 1)		
		kpi.transition()
				 .attr("opacity", 0)
