(function () {
	...
	var xScaleAssists = d3.scaleLinear()
		.range([0, width]);
	...
	var div = d3.select("#DeleAlliPLBar").append("div")
    .attr("class", "tooltipbar")
    .style("opacity", 0);
		
	// load data
	d3.csv("https://raw.githubusercontent.com/jakehughes1882/data/master/DeleAlliPL.csv", type, function(error, data) {	
		...
		// create svg
		var svg = d3.select("#DeleAlliPLBar")
			.append("svg")
    ...
    d3.select("#Goals").on("click", function() {
	...
	d3.select("#Assists").on("click", function() {
	    
		// domains
		xScaleAssists.domain(d3.extent(data, function(d) { return d.assists; })).nice();  
		//xScale.range([0, width-300]);  
		//var xAxis = d3.axisBottom()
		//			  .scale(xScaleAssists)  
		...
		svg.selectAll(".bar")
	    	.transition()	
				...
				.transition()			
				.attr("x", function(d) { return xScale(Math.min(0, d.goals)); })
				.attr("width", function(d) { return Math.abs(xScaleAssists(d.assists) - xScaleAssists(0)); })	

		});

	d3.select("#SPG").on("click", function() {
		...
		svg.selectAll(".bar")
	    	.transition()	
				...
				.attr("x", function(d) { return xScale(Math.min(0, d.goals)); })
				.attr("width", function(d) { return Math.abs(xScaleSPG(d.SPG) - xScaleSPG(0))/1.2; })	
